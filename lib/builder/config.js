...
const config = toml.parse(fs.readFileSync(resolve('.rabbit')))
...
class DllBlackList {
    ...
    set(names) {
        names.forEach(n => {
            if(includes(n, this.get())) return
            return config.dll.blackList.push(n)
        })
	const output = tomlify(config, null, 2)
        fs.writeFile(resolve('.rabbit'), output, err => {
            if(err) throw new err /* need emit error */
            console.log(names)
...
function webpackCannotResolve(msg) {
    return msg.match(/Can't resolve [\"|\']([\w|-]+)[\"|\'] in/)[1]
}
...
function rabbitCSSProps() {
	...
	["border-radius", "bdr", "bdrz", "bdra", "圆角"],

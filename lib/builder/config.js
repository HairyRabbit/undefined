...
import RabbitStylusDefineENV from './RabbitStylusDefineENV.js'
...
export function stylusOptions(env) {
  return {
	  include: env.paths,
	  use: [
      //transformComponentPathForStylus
      RabbitStylusUtilLoader,
      RabbitStylusDefineENV()
    ]
  }
}
...
function transformComponentPathForStylus(render) {
  let re = /@require\s*[\"|\'][u|c]:(\w+)(\[([\w|\s|\$|=|,|"|'|:]*)\])?[\"|\']/
  let matched = render.str.match(re)
  if(matched) {
    let namespace = 'util'
    let classname = matched[1]
    let isAutoLoad = matched[2]
    let params = matched[3]

    if(isAutoLoad) {
      render.str = render.str.replace(re, `@require "styles/${namespace}/${classname}"\nu-${classname}-export(${params})`)
    } else {
      render.str = render.str.replace(re, `@require "styles/${namespace}/${classname}"`)
    }
  }

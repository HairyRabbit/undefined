...
function webpackDllOptions(env) {
    return {
	entry: {
	    //alpha: [path.resolve(env.lib, 'dll', 'test')],
	    vendor: ['react', 'react-dom'],
	    //jquery: ['jquery']
	},
	module: [
...
function webpackOptions(env, entries) {
	...
	resolve: {
	    ...
	    ].concat(env.paths.concat('./').map(n => path.resolve(n)))
	},
	plugins: [
		...
		context : '.',
		manifest: require(path.resolve(env.dll, 'vendor-manifest.json'))
            }),
	    new webpack.DllReferencePlugin({
		context : '.',
		manifest: require(path.resolve(env.dll, 'tool-manifest.json'))
            }),
	    new HappyPack({
		id: 'js',
		threadPool: pool,
...
function normalizeFiles(env, filepath) {
    const webpackHMR = `webpack-hot-middleware/client?reload=true&overlay=true`
    const dll = 'module'

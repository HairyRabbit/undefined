...
function prepend3<T>(list: Array<T>, a: T, b: T, c: T): Array<T> {
  let init = list.slice(0, -3)
  return [].concat(a, b, c, init)
}

function prepend4<T>(list: Array<T>, a: T, b: T, c: T, d: T): Array<T> {
  let init = list.slice(0, -4)
  return [].concat(a, b, c, d, init)
}

function prepend5<T>(list: Array<T>, a: T, b: T, c: T, d: T, e: T): Array<T> {
  let init = list.slice(0, -5)
  return [].concat(a, b, c, d, e, init)
}

function prependn<T>(list: Array<T>, ...args: Array<T>): Array<T> {
  let init = list.slice(0, -n)
  return [].concat(args, init)
}


export function prepend<T>(list: Array<T>, ...args: Array<T>): Array<T> {
  let caller = null,
      target = and(args.length === 1, isArray(args[0])) ? args[0] : args
  ...
  target.unshift(list)
  // TODO Bug, list.length < target.length
  return caller.apply(null, target)
}
